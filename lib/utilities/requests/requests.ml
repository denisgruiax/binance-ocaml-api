...
let get url = 
  let json = let* uri = Lwt.return (Uri.of_string url) 
    ...
    in Lwt.return json'
  in Lwt_main.run json;;

let post url headers = 
  let json = let* (_, body) = Cohttp_lwt_unix.Client.post url ~headers ~body:(Cohttp_lwt.Body.empty)
    in let* body_string = Cohttp_lwt.Body.to_string body
    in let* json' = Lwt.return(Ezjsonm.from_string body_string)
    in Lwt.return json' in Lwt_main.run json;;